...
{
	public class BanditJump : BanditBaseState
	{

		public BanditJump(EnemyStateMachine s, Vector3 v, Transform transform, int id) : base(velocity: v, banditId: id)//=> _stateMachine = s; 
		{
			_banditID = id;
			_stateMachine = s;
			_velocity = v; //We want to carry this on between states.
			_transform = transform;
			_maxPowerStruckVelocityY = 7.0f; //10.0f for lowest impact. 14.0f for average. 16.0f for maximum impact.
