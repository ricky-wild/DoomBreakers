...
{
	public class NPCTravelling : BasicNPCBaseState
	{
		private bool _setFaceDir;
		private int _travelDir;
		public NPCTravelling(NPCStateMachine s, Vector3 v, Transform transform, int id) : base(velocity: v, npcId: id)//=> _stateMachine = s; 
		{
			_npcID = id;
			_stateMachine = s;
		...
		public override void IsTravelling(ref NPCAnimator animator, ref NPCSprite npcSprite)
		{
			//animator.Play("Run");//, 0, 0.0f);
			animator.PlayAnimation((int)NPCAnimID.Run);
